...
const els = {};
['main', 'leftNav', 'rightNav', 'searchInput', 'searchResults', 'menuToggle'].forEach((id) => {
  els[id] = mkEl();
});
const headEl = mkEl();
global.document = {
...
let CURRENT = '/';
global.location = { get pathname() { return CURRENT; }, origin: 'https://projectzomboid.gamewikihub.com' };
...
/* -------------------- route list -------------------- */
function allRoutes() {
  const routes = ['/', '/getting-started', '/about', '/privacy-policy', '/contact',
    '/guides', '/traits', '/occupations', '/skills', '/weapons', '/maps',
    '/build42', '/vehicles', '/mods', '/multiplayer', '/tips', '/patches'];
  const map = {
    guides: 'guides', traits: 'traits', occupations: 'occupations', skills: 'skills',
    weapons: 'weapons', maps: 'maps', build42: 'build42', vehicles: 'vehicles',
    mods: 'mods', multiplayer: 'multiplayer'
  };
...
function run() {
  ...
  if (!/<!-- pzw:head -->/.test(template)) {
    console.error('ERROR: index.html is missing the <!-- pzw:head --> marker. Aborting.');
    process.exit(1);
  }
